...
def create_contribution_df(file_name: str) -> 'pd':
    ...
    datetime_index_trades = pd.to_datetime(
        contribution_df['Date'], dayfirst=True)
...
def create_daily_contribution_df(contribution_df: 'pd', column_name: str) -> 'pd':
    ...
    for date in set_analysis_dates.date_range:
        ...
        daily_contribution_balance_df = pd.concat(
            [new_row_df, daily_contribution_balance_df])
    # Convert column from string to integer
    daily_contribution_balance_df[column_name] = (
        daily_contribution_balance_df[column_name].astype(int))
    # Add 'Date' label to the index column
    daily_contribution_balance_df = daily_contribution_balance_df.rename_axis(
        'Date', axis=1)
...
def sum_daily_contribution_df(file_name1: str, file_name2: str) -> 'pd':
    ...
    Requires a CSV file from 'create_daily_contribution_df' function as input.
    'Date' column  is the index, in datetime format,
    values are displayed as integers."""
    ...
    # Add Total Values
    df_total['Tot_Contribuciones_MXN'] = (
