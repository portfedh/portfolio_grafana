...
subtotals_df = db.create_df('outputs/daily_subtotals_PCL_AllAccounts.csv')
...
#########
# Output to CSV
filename = 'outputs/daily_percentages_PCL_GBM.csv'
subtotals_df.to_csv(filename, index=True, index_label='Date')

# Output to MySQL
table_name = 'daily_percentages_PCL_GBM'
subtotals_df.to_sql(
    name=table_name,
    con=engine,
    if_exists='replace',
